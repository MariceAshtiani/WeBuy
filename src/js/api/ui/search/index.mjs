...
export function setupSearch(listings, container) {
    ...
    searchForm.addEventListener("submit", function (event) {
        ...
        const filteredListings = listings.filter(function (listing) {
            ...
            const tagsMatch = Boolean(listing.tags.map(tag => tag.toLowerCase()).filter(tag => tag.includes(term)).length)

            return title.includes(term) || tagsMatch || seller.includes(term) || description.includes(term);
            //
            //
        })
